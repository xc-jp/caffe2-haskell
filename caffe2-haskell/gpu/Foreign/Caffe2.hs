...
#define READ_TENSOR(WS,CTYPE,NAME)                                                       \
  ...
  [C.block| void {                                                                       \
    ...
    Blob *blob = workspace->GetBlob(outName);                                            \
...
#define READ_CPU_TENSOR(WS,CTYPE,NAME)                                                   \
  Mutable.new (fromIntegral outputSize) >>= \vec ->                                      \
  [C.block| void {                                                                       \
    Workspace *workspace = static_cast<Workspace *>($(void *WS));                        \
    std::string outName($bs-ptr:NAME,$bs-len:NAME);                                      \
...
readCPUTensor
  ...
  -> Text                       -- ^ Name of tensor
  -> IO (Tensor sh e)
readCPUTensor (Workspace workspace) (ShapeElt outShape elt) out =
  case elt of
    SEltTypeFloat  -> READ_CPU_TENSOR(workspace,float,outBytes)
    SEltTypeDouble -> READ_CPU_TENSOR(workspace,double,outBytes)
    SEltTypeWord8  -> READ_CPU_TENSOR(workspace,uint8_t,outBytes)
    SEltTypeWord16 -> READ_CPU_TENSOR(workspace,uint16_t,outBytes)
    SEltTypeWord32 -> do
        sayErr "Warning: Caffe2 doesn't support Word32"
        sayErr "Reading the blob as Int32"
        READ_CPU_TENSOR(workspace,int32_t,outBytes)
    SEltTypeWord64 -> do
        sayErr "Warning: Caffe2 doesn't support Word64"
        sayErr "Reading the blob as Int64"
        READ_CPU_TENSOR(workspace,int64_t,outBytes)
    SEltTypeInt8   -> READ_CPU_TENSOR(workspace,int8_t,outBytes)
    SEltTypeInt16  -> READ_CPU_TENSOR(workspace,int16_t,outBytes)
    SEltTypeInt32  -> READ_CPU_TENSOR(workspace,int32_t,outBytes)
    SEltTypeInt64  -> READ_CPU_TENSOR(workspace,int64_t,outBytes)
    SEltTypeUnit   -> pure $ Tensor $ Acc.fromList outShape (replicate (arraySize outShape) ())
  where
    outputSize = fromIntegral $ arraySize outShape :: CInt
    outBytes = encodeUtf8 out
...
#define INPUT_CPU_TENSOR(WS,CTYPE,NAME,PTR,SIZE,DIMS)                                    \
 [C.block| void {                                                                        \
    Workspace *workspace = static_cast<Workspace *>($(void *WS));                        \
    int64_t *arrInputDims = $vec-ptr:(int64_t *DIMS);                                    \
    std::vector<int64_t> shape =                                                         \
...
inputCPUTensor
  ...
  -> Tensor sh e            -- ^ Tensor to load
  -> IO ()
inputCPUTensor (Workspace workspace) i tensor =
  case selt (Proxy :: Proxy e) of
    SEltTypeFloat ->
      let ptr = castPtr (toPtrs tensor)
       in INPUT_CPU_TENSOR(workspace,float,inputBytes,ptr,inputSize,inputDimsVec)
    SEltTypeDouble ->
      let ptr = castPtr (toPtrs tensor)
       in INPUT_CPU_TENSOR(workspace,double,inputBytes,ptr,inputSize,inputDimsVec)
    SEltTypeWord8 ->
      let ptr = castPtr (toPtrs tensor)
       in INPUT_CPU_TENSOR(workspace,uint8_t,inputBytes,ptr,inputSize,inputDimsVec)
    SEltTypeWord16 ->
      let ptr = castPtr (toPtrs tensor)
       in INPUT_CPU_TENSOR(workspace,uint16_t,inputBytes,ptr,inputSize,inputDimsVec)
    SEltTypeWord32 ->
      let ptr = castPtr (toPtrs tensor)
       in do
       sayErr "Caffe2 doesn't support Word32"
       sayErr "inputting tensor as Int32 instead"
       INPUT_CPU_TENSOR(workspace,int32_t,inputBytes,ptr,inputSize,inputDimsVec)
    SEltTypeWord64 ->
      let ptr = castPtr (toPtrs tensor)
       in do
       sayErr "Caffe2 doesn't support Word64"
       sayErr "inputting tensor as Int64 instead"
       INPUT_CPU_TENSOR(workspace,int64_t,inputBytes,ptr,inputSize,inputDimsVec)
    SEltTypeInt8 ->
      let ptr = castPtr (toPtrs tensor)
       in INPUT_CPU_TENSOR(workspace,int8_t,inputBytes,ptr,inputSize,inputDimsVec)
    SEltTypeInt16 ->
      let ptr = castPtr (toPtrs tensor)
       in INPUT_CPU_TENSOR(workspace,int16_t,inputBytes,ptr,inputSize,inputDimsVec)
    SEltTypeInt32 ->
      let ptr = castPtr (toPtrs tensor)
       in INPUT_CPU_TENSOR(workspace,int32_t,inputBytes,ptr,inputSize,inputDimsVec)
    SEltTypeInt64 ->
      let ptr = castPtr (toPtrs tensor)
       in INPUT_CPU_TENSOR(workspace,int64_t,inputBytes,ptr,inputSize,inputDimsVec)
    SEltTypeUnit -> pure ()
  where
    inputBytes = encodeUtf8 i
    inputSize :: CInt
